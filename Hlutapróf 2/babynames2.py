...
def call_functions(opened_file):
    ...
    boys_rank = find_percentile_rank(b_list_frequencies,frequency_boys)
    girls_rank = find_percentile_rank(g_list_frequencies,frequency_girls)
    print_func(print_list,boy_list,girl_list,frequency_boys,frequency_girls,boys_rank,girls_rank)

def find_frequency(boy_list,girl_list):
    '''A function which finds the total frequency of all boy names and then all of the girl names and returns seperate lists '''
    boy_frequency = sum(boy_list)
    girl_frequency = sum(girl_list)
    return boy_frequency, girl_frequency

def find_percentile_rank(list_frequencies,frequency):
    '''A function which finds the 50th percentile rank and returns it'''
    count_rank = 0
    sum_elements = 0
    half_frequency = frequency/2
    for elements in list_frequencies:
        sum_elements += elements
        count_rank += 1
        if sum_elements > half_frequency:
            return count_rank


def print_func(list1,list2,list3,frequency_boys,frequency_girls,boys_rank,girls_rank):
    ...
    print("50th percentile rank for boys:",boys_rank)
    print("50th percentile rank for girls:",girls_rank)
